...
#   # networking
#   create_drg       = true
#   drg_display_name = "chuncheon"

#   #   remote_peering_connections = var.connectivity_mode == "mesh" ? { for k, v in merge({ "admin" = true }, var.clusters) : "rpc-to-${k}" => {} if tobool(v) && k != "chuncheon" } : { "rpc-to-admin" : {} }
...
#     } if tobool(lookup(var.clusters, c) && c != "chuncheon")] : []
#   )

#   vcn_cidrs     = [lookup(lookup(var.cidrs, lower("chuncheon")), "vcn")]
#   vcn_dns_label = "chuncheon"
#   vcn_name      = "chuncheon"
...
#   count = tobool(lookup(var.clusters, "chuncheon")) ? 1 : 0

# }

# resource "oci_objectstorage_bucket" "thanos_chuncheon" {
#   compartment_id = var.compartment_id
#   name           = "${var.label_prefix}-${lookup(var.thanos, "bucket_name", "thanos")}"#   namespace      = lookup(var.thanos, "bucket_namespace")

#   provider = oci.chuncheon

#   count = tobool(lookup(var.clusters, "chuncheon", "false")) && tobool(lookup(var.thanos, "enabled", "false")) ? 1 : 0
# }

# module "chuncheon_drg" {
...
#   # drg parameters
#   drg_display_name = "chuncheon-drg"

#   drg_vcn_attachments = {
#     drg = {
#       vcn_id                    = module.chuncheon[0].vcn_id
...
#   count = tobool(lookup(var.clusters, "chuncheon", "false")) ? 1 : 0
#   providers = {
#     oci = oci.chuncheon
#   }
# }