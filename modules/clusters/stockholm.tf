...
# resource "oci_objectstorage_bucket" "thanos_stockholm" {
#   compartment_id = var.compartment_id
#   name           = "${var.label_prefix}-${lookup(var.thanos, "bucket_name", "thanos")}"
#   namespace      = lookup(var.thanos, "bucket_namespace")

#   provider = oci.stockholm

#   count = tobool(lookup(var.clusters, "stockholm", "false")) && tobool(lookup(var.thanos, "enabled", "false")) ? 1 : 0
# }

# module "stockholm_drg" {
...
#   # drg parameters
#   drg_display_name = "stockholm-drg"

#   drg_vcn_attachments = {
#     drg = {
#       vcn_id                    = module.stockholm[0].vcn_id
...
#   count = tobool(lookup(var.clusters, "stockholm", "false")) ? 1 : 0
#   providers = {
#     oci = oci.stockholm
#   }
# }