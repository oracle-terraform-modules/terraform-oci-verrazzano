...
# resource "oci_objectstorage_bucket" "thanos_hyderabad" {
#   compartment_id = var.compartment_id
#   name           = "${var.label_prefix}-${lookup(var.thanos, "bucket_name", "thanos")}"
#   namespace      = lookup(var.thanos, "bucket_namespace")

#   provider = oci.hyderabad

#   count = tobool(lookup(var.clusters, "hyderabad", "false")) && tobool(lookup(var.thanos, "enabled", "false")) ? 1 : 0
# }

# module "hyderabad_drg" {
...
#   # drg parameters
#   drg_display_name = "hyderabad-drg"

#   drg_vcn_attachments = {
#     drg = {
#       vcn_id                    = module.hyderabad[0].vcn_id
...
#   count = tobool(lookup(var.clusters, "hyderabad", "false")) ? 1 : 0
#   providers = {
#     oci = oci.hyderabad
#   }
# }